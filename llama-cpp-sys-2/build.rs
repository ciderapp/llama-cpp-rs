...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=LLAMA_SYCL_TARGET");
    println!("cargo:rerun-if-env-changed=LLAMA_SYCL_DEVICE_ARCH");
    println!("cargo:rerun-if-env-changed=LLAMA_SYCL_F16");
    println!("cargo:rerun-if-env-changed=LLAMA_SYCL_CC");
    println!("cargo:rerun-if-env-changed=LLAMA_SYCL_CXX");
    ...
    if cfg!(feature = "sycl") {
        ...
        if let Ok(target) = env::var("LLAMA_SYCL_TARGET") {
            config.define("GGML_SYCL_TARGET", target);
        }

        if let Ok(device_arch) = env::var("LLAMA_SYCL_DEVICE_ARCH") {
            config.define("GGML_SYCL_DEVICE_ARCH", device_arch);
        }

        if env::var("LLAMA_SYCL_F16").is_ok_and(|v| v == "1") {
            config.define("GGML_SYCL_F16", "ON");
        }

        if let Ok(cc) = env::var("LLAMA_SYCL_CC") {
            config.define("CMAKE_C_COMPILER", cc);
        }

        if let Ok(cxx) = env::var("LLAMA_SYCL_CXX") {
